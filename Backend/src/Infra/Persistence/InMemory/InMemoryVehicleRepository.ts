...
export class InMemoryVehicleRepository {
	...
	remove(vehicleId: string): void {
		this.vehicles = new Map(
			Array.from(this.vehicles).filter(
				([key, vehicle]) => vehicle.id !== vehicleId,
			),
		);
	}
	exists(id: string): Promise<boolean> {
		const res = Array.from(this.vehicles.values()).some(
	...
	findBySerialNumber(vehicleSerialNumber: string): any | null {
		Array.from(this.vehicles).filter(
			([key, vehicle]) => vehicle.id !== vehicleSerialNumber,
		);
	}
